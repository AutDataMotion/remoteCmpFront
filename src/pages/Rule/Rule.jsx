  ...
  renderText = () => {
    return (
      <div className="contentEditorHtmlOutput" data-spm-anchor-id="5176.12281976.0.i1.34164c2ajQFYls">
        ...
        <p>
          决赛实施：决赛队伍须在决赛日当天完成现场模型测试和加分赛两个环节。
          现场模型测试环节中，要求各参赛队伍携带笔记本电脑，利用竞赛组织方提供的华为云服务器进行算法模型环境搭建，基于相应的决赛测试数据现场测试模型，考察模型的精度指标。
        ...
        <p>
          根据竞赛决赛的队伍成绩，遴选各竞赛单元中排名前5的参赛队伍 (具体数量由竞赛组委会视参赛情况确定)，经审核合格后进入复审答辩阶段。
        </p>
        <p>
          答辩形式：复审答辩将以现场答辩的方式进行。
        </p>
        <p>
          材料提交：晋级复审答辩的团队需提前准备答辩材料，包括答辩PPT、算法模型源代码和相关文档、参赛总结、身份信息以及其他相关材料。
        </p>
        <p>
          答辩规则：① 每支参赛队伍面对评委有15分钟的陈述时间和10分钟的问答时间；② 评委根据参赛选手的技术思路、理论深度和现场表现等进行综合评分；③ 复审答辩将对参赛队伍的算法成绩和答辩成绩进行综合评估得出，并依据复审答辩结果拟定各参赛队伍获奖等级。
        </p>
        <p>
