  ...
  renderText = () => {
    return (
      <div className="contentEditorHtmlOutput" data-spm-anchor-id="5176.12281976.0.i1.34164c2ajQFYls">
        ...
        <p>
          现场算法模型测试环节中，要求各参赛队伍携带笔记本电脑，基于竞赛组织方提供的云服务器和相应测试数据现场测试算法模型，并进行方案效果展示。竞赛组织方将对模型现场解译和处理结果进行审查，并基于算法效率、模型精度、模型大小等指标加权，对算法模型性能进行综合评估与排名，具体评价方式将在决赛阶段公布。决赛加分赛环节中，由竞赛组织方提供华为Atlas 200 Developer Kit智能开发套件，参赛队伍将决赛算法模型移植于为Atlas 200 Developer Kit智能开发平台，实现算法模型的推理计算。
        ...
        <p>
          答辩规则：① 每支参赛队伍面对评委有15分钟的陈述时间和10分钟的问答时间；② 评委根据选手的技术思路、理论深度和现场表现等进行综合评分；③ 复审答辩分数由大赛评委会专家根据参赛队伍的决赛成绩和答辩成绩加权得出，具体方案将在决赛阶段公布。
        </p>
        <p>
